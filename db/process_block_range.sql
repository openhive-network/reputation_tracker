...
DROP FUNCTION IF EXISTS reptracker_app.process_block_range_data_b;
CREATE OR REPLACE FUNCTION reptracker_app.process_block_range_data_b(
  IN _first_block_num INT,
  IN _last_block_num INT,
  INOUT _account_reputations reptracker_app.AccountReputation[]
)
    RETURNS reptracker_app.AccountReputation[]
    LANGUAGE 'plpgsql'
    VOLATILE 
    SET from_collapse_limit = 16
    SET join_collapse_limit = 16
...
LOOP
  ...
  __prev_rep_delta :=  (__vote_data.prev_rshares >> 6)::BIGINT;
  __prev_rshares := __vote_data.prev_rshares;
  __rshares  := __vote_data.rshares;
  __author_id := __vote_data.author_id + 1;
  __voter_id := __vote_data.voter_id + 1;
  __author_rep := _account_reputations[__author_id].reputation;
  __voter_rep := _account_reputations[__voter_id].reputation;
  __implicit_author_rep := _account_reputations[__author_id].is_implicit;
  __implicit_voter_rep := _account_reputations[__voter_id].is_implicit;
...
IF NOT __implicit_author_rep AND __voter_rep >= 0 AND (__prev_rshares >= 0 OR (__prev_rshares < 0 AND NOT __implicit_voter_rep AND __voter_rep > __author_rep - __prev_rep_delta)) THEN
  ...
  IF __vote_data.voter_id = __vote_data.author_id THEN 
  ...
  END IF;
    ...
    _account_reputations[__author_id] := ROW(__vote_data.author_id, __author_rep, __implicit_author_rep, false)::reptracker_app.AccountReputation;
    __vote_data.__changed = true;
END IF;
...
IF __voter_rep >= 0 AND (__rshares >= 0 OR (__rshares < 0 AND NOT __implicit_voter_rep AND __voter_rep > __author_rep)) THEN
  ...
  _account_reputations[__author_id] := ROW(__vote_data.author_id, __new_author_rep, false, false)::reptracker_app.AccountReputation;
  __vote_data.__changed = true;

END IF;

IF __vote_data.__changed THEN 
  INSERT INTO reptracker_app.account_reputations
  (account_id, reputation, is_implicit)
  VALUES (__vote_data.author_id, _account_reputations[__author_id].reputation, _account_reputations[__author_id].is_implicit)
